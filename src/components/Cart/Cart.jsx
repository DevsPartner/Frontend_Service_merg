...
export default function CartPage() {
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#0f172a] flex items-center justify-center p-4">
        <div className="max-w-md w-full text-center bg-white dark:bg-gray-900 rounded-3xl p-8 border border-gray-100 dark:border-gray-800">
          <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-3">Fehler</h2>
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#0f172a] flex items-center justify-center p-4">
        <div className="max-w-md w-full text-center">
          ...
          <h2 className="text-3xl font-black text-gray-900 dark:text-white mb-3 tracking-tight">Dein Warenkorb ist leer</h2>
          <p className="text-gray-500 dark:text-gray-400 mb-8 font-medium">Es sieht so aus, als hättest du noch keine Produkte hinzugefügt.</p>
          <Link
            href="/products"
  ...
  // Checkout-Link handles guest state
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0f172a] py-12 transition-colors duration-300">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-4xl font-black text-gray-900 dark:text-white mb-2 tracking-tighter">Warenkorb</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
              ...
              >
                <div className="relative w-full sm:w-32 h-32 flex-shrink-0">
                  <img
                    ...
                    onError={(e) => (e.currentTarget.src = "https://placehold.co/400x400/1e293b/white?text=Tech")}
                  />
                </div>
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="text-xl font-bold text-gray-900 dark:text-white group-hover:text-blue-600 transition-colors">{item.name}</h3>
                      <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-1 mt-1 font-medium">{item.description || "Premium Technology"}</p>
                    </div>
                    <button
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 mt-6">
                    <div className="flex items-center bg-gray-100 dark:bg-gray-800 p-1 rounded-xl border border-gray-200 dark:border-gray-700">
                      <button onClick={() => updateQuantity(item.id, Math.max(1, item.quantity - 1))} className="w-8 h-8 flex items-center justify-center font-black transition-colors">–</button>
                      <span className="w-10 text-center font-black">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="w-8 h-8 flex items-center justify-center font-black transition-colors">+</button>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-gray-400 uppercase font-bold tracking-widest">Gesamtpreis</p>
                      <p className="text-lg font-black text-gray-900 dark:text-white">{((item.price || 0) * item.quantity).toFixed(2)} €</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white dark:bg-gray-900 rounded-3xl p-8 border border-gray-100 dark:border-gray-800 shadow-sm sticky top-28">
              <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-6 tracking-tight">Bestellübersicht</h2>
              <div className="space-y-4 mb-8">
                ...
                <div className="pt-6 border-t border-gray-100 dark:border-gray-800 flex justify-between items-end">
                  <span className="text-gray-900 dark:text-white font-black text-lg">Summe</span>
                  <span className="text-3xl font-black text-blue-600">{total.toFixed(2)} €</span>
                </div>
              </div>
              <Link href={checkoutHref} className="block w-full py-5 bg-blue-600 hover:bg-blue-700 text-white text-center font-bold rounded-2xl shadow-xl transition-all">Zur Kasse gehen</Link>
              {!user && <p className="mt-3 text-center text-xs text-gray-400 font-bold uppercase tracking-widest">Zum Checkout musst du dich einloggen</p>}
            </div>
          </div>
        </div>
      </div>
